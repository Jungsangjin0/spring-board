...
public class Pagination {
	...
	//PageInfo 객체를 리턴하는 메소드
	//currentPage와 listCount를 파라미터로 받는다.
	public PageInfo getPageInfo(int currentPage, int listCount) {
		 ...
		 * 0.9 더하는 이유 : (한 페이지 게시글 수가 10인데 1개라도 더 있다면 0.9를 더해서 한페이지를 더 만든다.
		 * )
		 * ex) 26 / 10 = 2.6,
		 * => 2.6 + 0.9 = 3.5
		 * => (int) 3.5 = 3
		 * 총 페이지 수 = 1, 2, 3
		 * */						
		maxPage = (int)((listCount / (double)boardLimit) + 0.9);
		 ...
		 * 
		 * */
		startPage = (((int)((currentPage / (double)pageLimit) + 0.9)) - 1) * pageLimit + 1;
		...
		/*
		 * 만약 페이징이 13까지 있다면, 20까지 표시되는 것이 아니라 13 까지만 표시된다.
		 * */
		if(maxPage < endPage) {
			endPage = maxPage;
		}
